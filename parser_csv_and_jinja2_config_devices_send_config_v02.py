...
def device_config(yaml_data,stored_credentials,x):
    for device in yaml_data['all']['sites']['hosts']:
        ...
        #print(command_in)
        #return device_in_site                                                                   # populate list for connecting
        #print(interface_config.split("\n"))
        con_device = ConnectHandler(**device_in_site)                                           # connect to each device
        output = con_device.send_config_set(x)                                    # config loopback interface with set of commands
        print(output)
